...
func ConnectToServer(endpoint string, opts ConnectToServerOpts) (*APIoverJSONRPC, error) {
	...
	ws.SubscribeHandler = stream.notifyConnected
	go ws.Dial(endpoint, reqHeader)

	var res APIoverJSONRPC
	res.C = jsonrpc2.NewConn(opts.Context, stream, jsonrpc2.HandlerWithError(res.handler))
	return &res, nil
}
...
func (stream *jsonRPCStream) notifyConnected() error {
	...
	stream.connectListeners = nil
	return nil
}

func (stream *jsonRPCStream) ensureConnected() error {
	listener, err := stream.getConnectListener()
	if err != nil {
		return err
	}
	if listener != nil {
...
func (stream *jsonRPCStream) getConnectListener() (<-chan struct{}, error) {
	stream.mu.Lock()
	defer stream.mu.Unlock()

	if stream.closed {
		return nil, recws.ErrNotConnected
	}
	if stream.ws.IsConnected() {
		return nil, nil
	}
	connectListener := make(chan struct{})
	stream.connectListeners = append(stream.connectListeners, connectListener)
...
func (stream *jsonRPCStream) WriteObject(obj interface{}) error {
	for {
		err := stream.ensureConnected()
		if err != nil {
			return err
		}
		err = stream.ws.WriteJSON(obj)
		if err == nil {
			return nil
		}
